...
For Atari Games, it makes more since to stack several recent frames since the agent need to know what is
happening and with only one frame it is hard to judge the current situation. So we have the `observ_stack_len`
parameter. 

```{r}
library(rlR)
env = makeGymEnv("Seaquest-v0", observ_stack_len = 4L, state_preprocess = list(fun = subsample))
env$overview()
```
