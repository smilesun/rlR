...
RLConf = R6Class("Conf",
  public = list(
    ...
    initialize = function() {
      self$static = data.table::copy(RLConfDefault)  # deep copy
    },
    ...
    register = function(namespace) {  # register namespace
      if(namespace %in% names(self$static)) return(FALSE)
      self$static[[namespace]] = list()
      return(TRUE)
    },

    attachname = function(namespace, list.input) {  # attach a new subspace of parameters
      if(namespace %in% names(self$static)) return(FALSE)
